...
int main()
{
	...
	while (1)
	{
		...
		{
			// //DAILY_ROUTINE
			// if (is_in_hand(decks, deck_count, CONTINUOUS_INTEGRATION) &&)
			// {

			// }
			// REFACTORING
			if (is_in_hand(decks, deck_count, REFACTORING) && is_in_hand(decks, deck_count, DEBT) && data.nb_release == 4 && best_possible_release(apps, decks, possible_moves, app_count, deck_count, 0) > 0)
		...
		else if (game_phase == "RELEASE")
		{
			...
			if (release < 5)
				release = best_possible_release(apps, decks, possible_moves, app_count, deck_count, 3);
			else
