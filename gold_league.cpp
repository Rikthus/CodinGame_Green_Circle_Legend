...
int optimized_integration(int *best_ressources, Deck hand, Deck automated)
{
    int card = -1;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            if (hand.cards_count[best_ressources[i]] > 1 && automated.cards_count[best_ressources[i]] > 1)
            {
        ...
        {
            if (hand.cards_count[best_ressources[i]] > 0 && automated.cards_count[best_ressources[i]] > 1)
            {
                card = best_ressources[i];
                break;
            }
...
int check_prices(const vector<Application>& apps, Deck hand, Deck automated)
{
    int best_price = 100;

    for (int i = 0; i < apps.size(); i++)
    {
        if (get_price(apps[i].need, hand, automated) < best_price)
            best_price = get_price(apps[i].need, hand, automated);
    }
    return (best_price);
}

int normal_move(Data data, Data ennemy)
{
    int move = -1;
    int possible_move = data.current_location + 1;
    ...
    while (move == -1)
    {
        ...
        {
            if (!opponent_is_near(ennemy, possible_move))
                move = possible_move;
        }
        possible_move++;
    }
...
void    take_CI(int *movement, int *routine, Data data, Data ennemy, Deck hand, int *location_cards)
{
    if (location_cards[5] == 0)
        return;
    else
    {
        if (data.daily_routine_cards > 0)
        {
            if (data.current_location > 5)
            {
                if (!opponent_is_near(ennemy, 5))
                    *movement = 5;
                else if (!opponent_is_near(ennemy, 6) && data.current_location != 6)
                {
                    *movement = 6;
                    *routine = 5;
                }
                else if (!opponent_is_near(ennemy, 4))
                {
                    *movement = 4;
                    *routine = 5;
                }
            ...
            {
                if (!opponent_is_near(ennemy, 5) && data.current_location != 5)
                    *movement = 5;
                else if (!opponent_is_near(ennemy, 6) && data.current_location != 6)
                {
                    *movement = 6;
                ...
                else if (!opponent_is_near(ennemy, 4) && data.current_location != 4)
                {
                    *movement = 4;
                    *routine = 5;
                }
        ...
        else
        {
            if (data.current_location > 5)
            {
                if (!opponent_is_near(ennemy, 5))
                    *movement = 5;
                else if (!opponent_is_near(ennemy, 6))
                {
                    *movement = 6;
                    *routine = 5;
                }
            ...
            else
            {
                if (!opponent_is_near(ennemy, 5) && data.current_location != 5)
                    *movement = 5;
                else if (!opponent_is_near(ennemy, 6) && data.current_location != 5)
                    *movement = 6;
                else if (!opponent_is_near(ennemy, 4) && data.current_location != 5)
                    *movement = 4;
                else if (data.current_location != 3)
                    *movement = 3;
                else
...
void    search_best_move(int *movement, int *routine, Data data, Data ennemy, Deck hand, Deck draw, Deck automated, Deck discard, Deck played, const vector<Application>& apps, int *location_cards, int *ressources_rank)
{
        // cerr << "DAILY:   " << data.daily_routine_cards << endl;
        // cerr << "auto:   " << count_auto_cards(automated) << endl;
        // cerr << "bonus:   " << possessed_card(draw, hand, discard, played, automated, BONUS) << endl;
    if (data.is_first == 0 && count_auto_cards(automated) < 2 && possessed_card(draw, hand, discard, played, automated, BONUS) > 3 && location_cards[5] > 0)
    {
        take_CI(movement, routine, data, ennemy, hand, location_cards);
        if (*movement == -1)
            *movement = normal_move(data, ennemy);
    }
    else if (count_auto_cards(automated) < 2 && possessed_card(draw, hand, discard, played, automated, BONUS) > 3 && location_cards[5] > 0)
    {
        take_CI(movement, routine, data, ennemy, hand, location_cards);
        if (*movement == -1)
            *movement = normal_move(data, ennemy);
    ...
    else
    {
        ...
        {
            // if (check_possible_release(apps, hand, data, ennemy) < 4)
        ...
        {
            // if (check_possible_release(apps, hand, data, ennemy) < 1)
...
int main()
{
    ...
    while (1)
    {
        ...
        //         movevement = 6;
        //     else if (data.current_location != 4 &&  data.current_location != 5 && possessed_card(draw, hand, discard, played, automated, 5) < 3 && count_auto_cards(automated) < 3 && possessed_card(draw, hand, discard, played, automated, BONUS) > 1)
        //         movevement = 4;
        //     else if (data.current_location != 5 && possessed_card(draw, hand, discard, played, automated, 5) < 3 && count_auto_cards(automated) < 3 && possessed_card(draw, hand, discard, played, automated, BONUS) > 1)
        //         movevement = 5;
        //     else if (data.current_location != 6 && possessed_card(draw, hand, discard, played, automated, 5) < 3 && count_auto_cards(automated) < 3 && possessed_card(draw, hand, discard, played, automated, BONUS) > 1)
        //         movevement = 6;
        //     else
        //     {
        ...
        //         else
        //         {
        //             if (data.current_location != 4 &&  data.current_location != 5 && possessed_card(draw, hand, discard, played, automated, 5) < 3 && count_auto_cards(automated) < 3)
        //                 movevement = 4;
        ...
        //         }
        //     }
        //     if ((movevement == 4 || movevement == 5 || movevement == 6) && data.daily_routine_cards > 0 && location_cards[5] > 0 && possessed_card(draw, hand, discard, played, automated, 5) < 2 && count_auto_cards(automated) < 2)
        //         card_num = 5;
        ...
        if (game_phase == "MOVE")
        {
            ...
            if (data.turn == 0)
            {
                ...
                search_best_move(&movement, &routine, data, ennemy, hand, draw, automated, discard, played, apps, location_cards, ressources_rank);
            if (movement != -1 && routine != -1 && data.daily_routine_cards != 0)
                cout << "MOVE " << movement << " " << routine << endl;
            else if (movement != -1)
                cout << "MOVE " << movement << endl;
            else
                cout << "RANDOM" << endl;
            data.current_location = movement;
        }
        //GIVE
        ...
        else if (game_phase == "PLAY_CARD")
        {
            if (data.nb_release < 4)
            {
                ...
                if (is_in_hand(decks, deck_count, CODE_REVIEW) && possessed_card(draw, hand, discard, played, automated, BONUS) < 4 && count_auto_cards(automated) < 3)
                ...
                else if (is_in_hand(decks, deck_count, CONTINUOUS_INTEGRATION) && optimized_integration(ressources_rank, hand, automated) > 0)
                    cout << "CONTINUOUS_INTEGRATION " << optimized_integration(ressources_rank, hand, automated) << endl;
                else if (is_in_hand(decks, deck_count, TRAINING))
                    cout << "TRAINING" << endl;
                else if (is_in_hand(decks, deck_count, CODING))
                    cout << "CODING" << endl;
                else if (is_in_hand(decks, deck_count, ARCHITECTURE_STUDY) && data.architecture_study_cards < 4)
                ...
                else if (is_in_hand(decks, deck_count, DAILY_ROUTINE) && data.daily_routine_cards < 1)
            ...
            {       
                if (is_in_hand(decks, deck_count, CONTINUOUS_INTEGRATION) && is_in_hand(decks, deck_count, BONUS) && !is_release_possible(hand, automated, apps, 0))
                    cout << "CONTINUOUS_INTEGRATION 8" << endl;
                else if (is_in_hand(decks, deck_count, CONTINUOUS_INTEGRATION) && optimized_integration(ressources_rank, hand, automated) > 0 && !is_release_possible(hand, automated, apps, 0))
                    cout << "CONTINUOUS_INTEGRATION " << optimized_integration(ressources_rank, hand, automated) << endl;
                else if (is_in_hand(decks, deck_count, TRAINING) && !is_release_possible(hand, automated, apps, 0))
                    cout << "TRAINING" << endl;
                else if (is_in_hand(decks, deck_count, ARCHITECTURE_STUDY) && !is_release_possible(hand, automated, apps, 0) && data.architecture_study_cards < 3)
                    cout << "ARCHITECTURE_STUDY" << endl;
                else if (is_in_hand(decks, deck_count, TRAINING) && !is_release_possible(hand, automated, apps, 0))
                    cout << "TRAINING" << endl;
                    ...
                    cout << "REFACTORING" << endl;
                else if (is_in_hand(decks, deck_count, DAILY_ROUTINE) && data.daily_routine_cards < 1 && !is_release_possible(hand, automated, apps, 0))
                    cout << "DAILY_ROUTINE" << endl;
                else
                    cout << "WAIT" << endl;
            }
        ...
        else if (game_phase == "RELEASE")
        {
            ...
            else if (data.nb_release < 4)
            {
                for (int i = 0; i < apps.size(); i++)
                {
                    // if (get_price(apps[i].need, hand, automated) < 5)
                    // {
                    //     release_id = apps[i].id;
                    //     break;
                    // }
                    // if (get_price(apps[i].need, hand, automated) < 4)
                    // {
                    //     release_id = apps[i].id;
                    //     break;
                    // }
                    if (get_price(apps[i].need, hand, automated) < 3)
                    {
                        release_id = apps[i].id;
                        break;
                    }
                    ...
                    if (get_price(apps[i].need, hand, automated) < 1)
                    {
                        release_id = apps[i].id;
                        break;
                    }
